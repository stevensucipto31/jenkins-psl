...
def merge(Pipeline pipeline){
  ...
  def pr_merge_response = httpRequest authentication: 'github-personal', httpMode: 'PUT', url: "https://api.github.com/repos/${pipeline.git_user}/${pipeline.repository_name}/pulls/${pipeline.pr_num}/merge", validResponseCodes: '200,405,409', wrapAsMultipart: false
  ...
  if (parsed_pr_merge_response.containsKey("merged")){
      if ("${parsed_pr_merge_response['merged']}" == "true") {
       ...
       def pr_check_response = httpRequest authentication: 'github-personal', httpMode: 'GET', url: "https://api.github.com/repos/${pipeline.git_user}/${pipeline.repository_name}/pulls/${pipeline.pr_num}", validResponseCodes: '200,405,409', wrapAsMultipart: false
